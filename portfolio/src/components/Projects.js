...
export default function Projects() {
  ...
  const [mobileVisible, setMobileVisible] =
    useState(MOBILE_PER_PAGE);

  const showMoreWeb = () => {
    setWebVisible((prev) =>
      Math.min(prev + WEB_PER_PAGE, webProjects.length)
    );
  };

  const showMoreGame = () => {
    setGameVisible((prev) =>
      Math.min(prev + GAME_PER_PAGE, gameProjects.length)
    );
  };

  const showMoreMobile = () => {
    setMobileVisible((prev) =>
      Math.min(prev + MOBILE_PER_PAGE, mobileProjects.length)
    );
  };

  const visibleWebProjects = webProjects.slice(0, webVisible);
  const visibleGameProjects = gameProjects.slice(0, gameVisible);
  const visibleMobileProjects = mobileProjects.slice(
    0,
    mobileVisible
  );
